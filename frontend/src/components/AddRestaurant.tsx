  ...
  return (
    <div className='min-h-screen bg-gray-50 px-4 py-6' >
      <div className='mx-auto max-w-lg rounded-xl bg-white p-6 shadow-sm space-y-5 ' >
        ...
        <input type="file" id="image" accept="image/*" onChange={(e) => setImage(e.target.files?.[0] || null)} className='hidden' />
        <div className='flex items-center gap-3 rounded-lg border p-3' >
          ...
          <div className='text-xs w-[90%] h-[90%]' >
            {
              loadingLocation ? "Fetching Location..." : location?.formatedAddress || "Please Turn on Location"
            }
          </div>
        </div>
        <button className=' text-white py-2 px-4 rounded-md bg-[#ca1b1b] transition-all duration-300 ease-in-out   hover:bg-[#2563eb]' onClick={handleSubmit} disabled={submiting}  >
